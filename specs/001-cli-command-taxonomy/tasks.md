...
|---|---|---|---|---|
| T0 | Owner answers Q1 (mandatory datastore token) and Q2 (fan-out cost + staleness defaults); grammar updated | — | Both recorded in `spec.md`; `grammar.md` reflects them | — |
| T1 | Owner approves the grammar and the acceptance table | T0 | Recorded approval; epic #70's gate satisfied | #70 |
| T2 | CLI name/alias contract on `Resource`, per-scope, uniqueness checked at import | T1 | `show appliance X deployment` resolves without `appliance/`; synthetic collision fails at startup | #77 |
...
| T5 | Outcome classifier + renderer for the eleven outcomes, human and JSON | T1 | One test per outcome, both modes; `{}`/`None`/`""`/204/`[]` each intentional | #78 |
...
| T12 | Removal boundary decided and documented in `--help` and the constitution record | T1 (Q3) | Aliases carry an expiry | #74 |
...
* **T5–T7 (#78) do not depend on the new grammar** and could land first if the
  gate stalls. The silent-command bug is live today and its fix is valuable
  under either grammar. This is the one part of the epic worth doing out of
  order.
